...
####################################################################################
####################################################################################
#PLATFORM:  Agilent-028005 SurePrint G3 Mouse GE 8x60K Microarray 
#B cells induced with OSKM (not pulsed)
####################################################################################
####################################################################################


current_gse = "gse46321"
...
#STEP 2: subset data
current_gse_phenoData = current_gse_phenoData[c(11:20,37:44),]
current_gse_assayData = current_gse_assayData[,c(11:20,37:44)]
...
visualstack = stack(current_gse_assayData_fd)
ggplot(visualstack, aes=(x=values)) + geom_density(aes(x=values, group=ind, color=ind)) + 
  labs(colour = "days", title = paste("density before B cells", current_gse, sep = " "))
...
visualstack = stack(current_gse_assayData_fd)
ggplot(visualstack, aes=(x=values)) + geom_density(aes(x=values, group=ind, color=ind)) + 
  labs(colour = "days", title = paste("density after scaling B cells", current_gse, sep = " "))
...
visualstack = stack(current_gse_assayData_fd)
ggplot(visualstack, aes=(x=values)) + geom_density(aes(x=values, group=ind, color=ind)) + 
  labs(colour = "days", title = paste("density after quantile normalization B cells", current_gse, sep = " "))
...
genes_table = getBM(attributes=c('agilent_sureprint_g3_ge_8x60k','entrezgene_id'), mart = ensembl)
genes = genes_table
current_gse_assayData = data.frame(current_gse_assayData)
entrez_vec = c()
for (i in current_gse_featureData$NAME){
  print(i)
  if (i %in% genes$agilent_sureprint_g3_ge_8x60k &&
      length(genes[genes$agilent_sureprint_g3_ge_8x60k == i, 2]) == 1){
    entrez_vec = c(entrez_vec, genes[genes$agilent_sureprint_g3_ge_8x60k == i, 2])
  }else{
    entrez_vec = c(entrez_vec, NA)
  }
}
...
cluster_plot = ggplot(cluster_values, aes(cluster_values[,1], cluster_values[,2])) +
  ...
  labs(colour = "days", title = paste("PCA plot","B cells", current_gse, sep = " "))
cluster_plot
...
ggplot(visualstack, aes=(x=values)) + geom_density(aes(x=values, group=ind, color=ind)) + 
  labs(colour = "days", title = paste("final density B cells", current_gse, sep = " "))


#STEP 9: limma
temp = strsplit(as.character(current_gse_phenoData$title), ' ')
days_vec = c()
for (i in temp){
  days_vec = c(days_vec, i[4])
}
days_vec[6] = '96,'
days_vec = sub(',', '', days_vec)
...
ggplot(first_entity_df, aes(day, first_entity_df, color = day))  + geom_point() +
  labs(colour = "days", x = "age", y = "expression", title = paste("entrez ID B cells", first_entity, current_gse, sep = " ")) +
  scale_x_continuous(labels = as.character(days_vec), breaks = days_vec)

save(top_genes_limma,file="gse46321/top_genes_Bcells.Rda")
